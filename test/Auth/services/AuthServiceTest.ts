...
test.serial("logins for web and return tokens and the user instance", async t => {
  ...
  const loginDetails = await AuthService.login("nickname@example.com", "123456", DeviceType.BROWSER, "someRandomString")
...
test.serial("creates a proper jwt token", async t => {
  ...
  const expireAt = moment()
    .add(1, "hour")
    .unix()
  const loginDetails = await AuthService.createJwtToken(user as any, device as any, expireAt)
