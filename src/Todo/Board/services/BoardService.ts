...
import { BoardUser, BoardUserType } from "../../../models/BoardUser"
import { BoardRepository } from "../repositories/BoardRepository"

export class BoardService {
  static validateUserOwnsBoard = async (user: User, boardId: string): Promise<boolean> => {
  ...
  static validateUserIsMemberOfBoard = async (user: User, boardId: string): Promise<boolean> => {
    const board = await Board.findOne({
      include: [
        {
          model: User,
