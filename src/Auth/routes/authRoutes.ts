...
export const authRoutes = (fastify: FastifyInstance) => {
  ...
  const authOpt = { preHandler: [AuthMiddleware.isJwtTokenValid] }
  ...
  fastify.post("/api/v1/auth/updateFirebaseCloudToken", authOpt, AuthController.updateFirebaseCloudToken)
  ...
  fastify.post("/api/v1/auth/logout", authOpt, AuthController.logout)
