...
export class ResetPasswordService {
  ...
  static sendResetEmail = async email => {
    const emailExists = await ResetPasswordService.validateEmailExists(email)
    if (!emailExists) {
      throw new EmailNotFound()
    }
    const readFile = util.promisify(fs.readFile)
    const templateString = await readFile(path.resolve(__dirname, "../templates/emails/resetEmail.ejs"), "utf8")
  ...
  static createResetLink = (email): string => {
